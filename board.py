...
class Board:
    def __init__(self, rows: int, cols: int, mines: int):
        ...
        if mines > ((rows * cols) - 9):
            raise ValueError(f"Слишком много мин для поля {rows}x{cols}")
        self._cols = cols
        self._rows = rows
        self._mines = mines
    ...
    def tile_open(self, i, j) -> List[BoardTile]:
        ...
        if self._tiles[i][j].type == BoardTile.mine:
            # иначе при ошибке на последнем ходе - говорит ПОБЕДА
            self._is_game_finished = False
    ...
    def array(self) -> np.ndarray:
        return np.array([[tile.number for tile in row] for row in self._tiles])

    @property
    def array_solution(self) -> np.ndarray:
        return np.array([[tile.number for tile in row] for row in self._board])

    def tile_has_neighbour(self, row: int, col: int) -> bool:
        for i in [row + 1, row - 1, row]:
            for j in [col + 1, col - 1, col]:
                if (
                    self.tile_valid(i, j)
